...
class Trader(object):
    ...
    def __init__(self):
        ...
        self.oldTrend = ''
        self.strength = 0
        self.walletUSD = 25.00000
        self.walletBTC = 0.00000
    ...
    def webSocket(self):
        self.sock = poloniex.PoloniexSocketed()
        self.sock.startws(subscribe={'ticker': self.on_ticker})
        while True:
            poloniex.sleep(5)
        self.sock.stopws(10)
    ...
    def trader(self):
        ...
        self.walletBTC = balance['BTC']
        self.walletUSD = balance['USDT']

        trades = self.polo.returnTradeHistory(currencyPair='USDT_BTC')
        print("trades:", trades)
        self.orders = orders
    ...
    def load_raw_record(self, indate, inopen, inclose):
        self.rawDate = np.append(self.rawDate, indate)
        self.date = np.append(self.date, datetime.datetime.fromtimestamp(indate))
        self.ask = np.append(self.ask, inclose)
        self.bid = np.append(self.bid, inopen)
    ...
    def on_ticker(self,tickData):
        if tickData[0]==121:
            rawData = [float(dat) for dat in tickData]
            self.tick[int(rawData[0])] = {'id': int(rawData[0]),
                                       'last': rawData[1],
                                       'lowestAsk': rawData[2],
                                       ...
                                       'isFrozen': int(rawData[7]),
                                       'high24hr': rawData[8],
                                       'low24hr': rawData[9]
                                       }
            self.on_usdt_btc_tick(self.tick[121])

    def on_usdt_btc_tick(self,tick):
        current_timestamp = int(datetime.datetime.utcnow().timestamp())
        print("tick-->", str(current_timestamp), str(self.rawDate[self.toWhat]), "self.toWhat:", self.toWhat)
        print("tick",tick)
        append_to_file(self.filename, current_timestamp, tick['highestBid'], tick['lowestAsk'],
                       0)
        self.load_raw_record(current_timestamp, tick['highestBid'], tick['lowestAsk'])
        print("bid:", tick['highestBid'], "ask:", tick['lowestAsk'])
        self.evaluate(self.toWhat)
        self.toWhat += 1
